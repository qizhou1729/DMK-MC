...
struct MetropolisMonteCarloPDMK{T, M}
    temperature::T
    trial_moves::M
    trial_args::Dict
    tree::Tree
...
function MetropolisMonteCarloPDMK(; temperature, trial_moves, trial_args, tree::Tree)
    return MetropolisMonteCarloPDMK(temperature, trial_moves, trial_args, tree)
end
...
@inline function Molly.simulate!(sys::System,
    ...
    E_pdmk_old = PDMK4MC.eval_energy(sim.tree) * 1.0u"kJ/mol"
    ...
    for step_n in 1:n_steps
        ...
        dE_pdmk = PDMK4MC.eval_shift_energy(sim.tree, idx, shift_vec[1].val, shift_vec[2].val, shift_vec[3].val) * 1.0u"kJ/mol"
